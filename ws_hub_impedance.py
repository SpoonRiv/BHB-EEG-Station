...
class ImpedanceWsHub:
    """
    阻抗 WebSocket 广播枢纽。

    设计目标：
    - 阻抗数据通常低频（~1-10Hz），UI 只需要最新状态；
    - 队列满时覆盖旧数据，避免积压与延迟扩散；
    - 对慢连接设置发送超时并自动剔除。
    """

    def __init__(self, cfg: ImpedanceWsHubConfig):
        self._cfg = cfg
        self._clients: List[WebSocket] = []
        self._queue: Optional["asyncio.Queue[Optional[List[float]]]]"] = None
    ...
    def enqueue_latest(self, sample: List[float]) -> None:
        if self._stopping:
            return
        ...
        while True:
            try:
                ...
                if not self._clients:
                    continue
                payload: Any = {"type": "impedance_data", "data": {"ts": float(time.time()), "values": item}}
